...
def makelnks(name, path, fileName, iconpath):
    ...
    shortcut.SetWorkingDirectory (os.path.dirname(os.path.abspath(__file__))+path)
    shortcut.SetDescription (name)
    shortcut.SetIconLocation (os.path.dirname(os.path.abspath(__file__))+path+iconpath, 0)
...
def main():
    ...
    makelnks("Archtimer - Config", "\\Archtimer\\", "guiWin.py", "cfg\\iconeC.ico")
    makelnks("Archtimer - Pastas", "\\change_dir_files\\", "main.py", "iconeDir.ico")
